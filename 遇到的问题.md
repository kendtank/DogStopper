...
2. 音频预处理模块的二次优化点：
   2.1 hann加窗改为dsp库实现 dsps_mul_f32(frame_in, hann_win, frame_out, FRAME_SIZE, 1, 1, 1);窗口长度400samples 步长160samples 加窗。 从952us优化到627us

## 结论
截至 2025 年，Espressif 仅在 ESP-IDF 框架中提供对 ESP-NN 加速的完整、可靠支持。Arduino 框架（包括 PlatformIO）没有官方支持的 TFLite Micro + ESP-NN 集成方案。已经验证过，无法配置成功，第三方库要集成需要花费大量时间，且无法保证正确性。
