...
void fft_power_init(int nfft)
{
    ...
    // 分配内部 RAM 缓冲（速度快）
    fft_input = (float *)heap_caps_malloc(sizeof(float) * fft_size * 2, MALLOC_CAP_INTERNAL | MALLOC_CAP_32BIT);  // 复数数组：实部/虚部交替，使用32位对齐， 4字节。
    ...
    // 初始化 ESP-DSP FFT 表
    dsps_fft2r_init_fc32(NULL, fft_size);  // 只需要fft_size大小的size
...
int fft_power_compute(const float *frames_in, int num_frames, int frame_size, int nfft, float *power_out)
{
    ...
    for (int f = 0; f < num_frames; f++) {
        ...
        memset(fft_input, 0, sizeof(float) * 2 * nfft);
        // 把 frame 拷贝到偶数位（real），多余的补0
        for (int i = 0; i < frame_size; ++i) {
            fft_input[2 * i] = frame[i]; // real
        ...
        dsps_bit_rev_fc32(fft_input, nfft);  // 位逆序调整   自动选择了加速版
        // dsps_bit_rev_lookup_fc32(fft_input, nfft, bitrev_table);  // 位逆序 (加速)
