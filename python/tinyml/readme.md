...
解释点：虽然[-1,1] 属于 [-128,127]， 但是：[-1,1] 是浮点数区间，不是 int8 的区间！！如果强制转换为int8，那么就是信息压缩，而不是有效的缩放了。要知道：归一化的 [-1,1] 是浮点空间， int8 是 256 个离散整数、 两者完全不同、不能混用
...
### 更新：1205 
1. 模型更新和优化
    - 介于模型实际的效果，发现：模型对音乐的鼓点声容易误判，以及对狗吠的类型识别度不够，所以需要做以下优化：
    - a. 模型结构优化：
    - b. 模型输入特征的优化：
    - c. 数据集的扩充和标准化：
    - d. 量化数据集的准备以及要求：
