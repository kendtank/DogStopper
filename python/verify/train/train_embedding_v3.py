...
def build_tflm_dog_bark_micro_lite_32d(mode="train"):
    """
    mode: "train" -> outputs normalized embedding
          "export" -> outputs raw embedding (no L2)
    """
...
# def build_audio_dataset(dataset_root, split=(0.7,0.2,0.1), min_extensions=('.WAV','.wav','.Wave','.wave')):
#     all_items = []
...
#         wav_files = [f for f in os.listdir(full_dir) if f.endswith(min_extensions)]
...
#
#     # TODO 不能直接随机打乱，应该是先按照狗id内部的滑片数量，分配好train和val，再随机打乱
